 ...
 * Only modify function derivative().
 * Two problems remain: too many characters in one line.
 * And too many lines in one function.
 */
public final class Myjavahello {
    /**
     * private constructor.
     */
    private Myjavahello() {
     ...
     * @return 求导结果.
     */
    static String derivative(final String expr, final String x) {
        String str2 = expr;
        String dan, news2 = "", resu = ""; //dan为开始的单项式，news2为新多项式
        if ((x.toLowerCase()).equals(resu.toLowerCase())) {
            System.out.println("variable is null");
            return str2;
        }
        int sit, i, f = 0, addsum, subnum, num, mi, xishu, fine, count = 0;
        //非法情况
        for (i = 0; i < x.length(); i++) {
            boolean b = ('a' <= x.charAt(i) && 'z' >= x.charAt(i))
                    || ('A' <= x.charAt(i) && 'Z' >= x.charAt(i));
            if (b) {
                f += 0;
            } else {
                f += 1;
        ...
        do {
            ...
            count++;
            if (count > 1) {
            addsum = str2.substring(1).indexOf('+');
            subnum = str2.substring(1).indexOf('-');
            } else {
                addsum = str2.indexOf('+');
                subnum = str2.indexOf('-');
                if (subnum == 0) {
            ...
            if (sit == -1) {
                ...
                //若变量在单项式末尾,则取变量前的所有内容为导数
                resu = dan.substring(0, dan.length() - x.length() - 1);
            } else if (!dan.substring(dan.length() - x.length()).equals(x)) {
                if (dan.charAt(dan.indexOf(x) + x.length()) != '^') {
                    ...
                    if (dan.indexOf(x) == 0) {
                        ...
                        resu = dan.substring(0, dan.indexOf(x) - 1)
                                + dan.substring(dan.indexOf(x)
                                    + x.length());
                    }
                } else {
                    if (dan.charAt(0) >= '0' && dan.charAt(0) <= '9') {
                        //开头有系数
                        if ((dan.charAt(dan.indexOf(x) + x.length() + 1) == '2')
                                && (dan.indexOf(x) + x.length() + 1 == dan.length() - 1)
                                || (dan.indexOf(x) + x.length() + 1 != dan.length() - 1
                                && dan.charAt(dan.indexOf(x) + x.length() + 2) == '*')) {
                            //若x为二次幂
                            for (i = 0; i < dan.length() && dan.charAt(i) >= '0' && dan.charAt(i) <= '9';) {
                                i++;
                            }
                            mi = Integer.parseInt(dan.substring(0, i)); //系数
                            resu = String.valueOf(mi * 2)
                                    + dan.substring(i, dan.indexOf(x))
                                    + x
                                    + dan.substring(dan.indexOf(x)
                                        + x.length()
                                        + 2);
                        } else {
                            for (i = dan.indexOf(x) + x.length() + 1; i < dan.length() && dan.charAt(i) != '*';) {
                                i++;
                            }
                        ...
                        if ((dan.charAt(dan.indexOf(x) + x.length() + 1) == '2')
                            ...
                            mi = Integer.parseInt(dan.substring(dan.indexOf(x)
                                    + x.length()
                                    + 1, i)); //幂次
                            if (fine == 1) {
                                news2 = news2 + '-';
                            }
                            resu = String.valueOf(mi)
                                    + '*'
                                    + dan.substring(0, dan.indexOf(x))
                                    + x
                                    + '^'
        ...
        if (news2.charAt(news2.length() - 1) == '+'
                || news2.charAt(news2.length() - 1) == '-') {
    ...
    public static void simplify(String str2,  String x)
    {
        ...
        for (j = 0;j<x_n.length();j++)
        {
            if (x_n.charAt(j) == '=')
            {
                ...
                {
                    final boolean b = 'a' <= va.charAt(i) && 'z' >= va.charAt(i) || ('A' <= va.charAt(i) && 'Z' >= va.charAt(i));
                    if (b) {
                        f += 0;
                    } else {
                        f += 1;
        ...
        do
        {
            ...
            if (result.length() >= 2)
            {
                ...
                } else if (result.substring(0, 2).equals("+-")) {
...
public static String expression(final String str1t, final String str2)
{        String str1 = str1t;
        String s2;
        String z;
        String sh;
        ...
        String s_re = "";
        String str3 = "";
        int i, j, h = 0, add_num, r, a, sub_num, num, q, xi = 1, n = 0, begin, kk = 0, sit, mn = 0, count, nn, p = 0, m = 0, biao, pq = 0, ch = 0, xis, xi_n;
        ...
        final String prompt = "polynomial is illegal";
        if (str1.equals(""))
        {
        ...
        do
        {
            ...
            final boolean tempdan = !dan.equals("");
            if (tempdan){
            dan = String.valueOf(xi)+'*'+dan;} else {
                dan = String.valueOf(xi);
            }
            dans = dan;
            //System.out.println(dan);
            for (int it = 0;it<dan.length();it++)//合并字母
            {
                //System.out.println("***");
                //System.out.println(i);
                if (dan.charAt(it) >= 'a' && dan.charAt(it) <= 'z' || dan.charAt(it) >= 'A' && dan.charAt(it) <= 'Z')
                {
                    //System.out.println(i);
                    for (j = it;(j<dan.length() && (dan.charAt(j) >= 'a' && dan.charAt(j) <= 'z' || dan.charAt(j) >= 'A' && dan.charAt(j) <= 'Z'));j++) {
                        ;
                    }
                    //System.out.println(j);
                    var = dan.substring(it, j);
            ...
            do{
                ...
                for (q = 1;q <= str3.length();q++)
                {
                    if ((q<str3.length() && str3.charAt(q) == ' ') || (q == str3.length()))
                    {
                        ...
                        h = 0;
                        rnlong = s_rnd;
                        for (p = 0;p<rnlong.length();p++)
                        {
                            if (rnlong.charAt(p) >= 'a' && rnlong.charAt(p) <= 'z' || rnlong.charAt(p) >= 'A' && rnlong.charAt(p) <= 'Z')
                            {
                                for (r = p;r<rnlong.length() && rnlong.charAt(r) != '*';r++) {
                                    ;
                                }
                                va_n = rnlong.substring(p,  r);//变量
                                if (re_n.indexOf(va_n) == -1)
                                {
                                    h = 1;
                                    break;
        ...
        for (i = 0;i<s_re.length();i++)
        {
                if ((s_re.charAt(i) == '1') && (i == 0 || s_re.charAt(i-1)<'0' || s_re.charAt(i-1)>'9') &&  i+1<s_re.length() && s_re.charAt(i+1) == '*')
                {
                    s_re = s_re.substring(0, i)+s_re.substring(i+2);
