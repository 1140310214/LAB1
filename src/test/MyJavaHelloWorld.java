...
public class MyJavaHelloWorld {
	...
	{
		int sit, i, f = 0, addNum, subNum, num;
		int mi, xishu, fine, count = 0;
		...
		do
		{
			...
			if (count > 1)
			{
			addNum = str2.substring(1).indexOf('+');
			...
			else if (dan.substring(
					dan.length()-x.length()).equals(x))//若变量在单项式末尾
			{
				resu = dan.substring(
						0, dan.length()-x.length()-1);//则取变量前的所有内容为导数	
			}
			else if (!dan.substring(
					dan.length()-x.length()).equals(x))
			{
				if (dan.charAt(
						dan.indexOf(x) + x.length()) != '^')
				{
					...
					else
					{
						resu = dan.substring(0, dan.indexOf(x)-1) 
								+ dan.substring(dan.indexOf(x) + x.length());
					}
				}
				else
				{
					if (dan.charAt(0) >= '0' 
							&& dan.charAt(0) <= '9')//开头有系数
					{
						if ((dan.charAt(dan.indexOf(x) + x.length() + 1) == '2') 
								&& (dan.indexOf(x) + x.length() + 1 == dan.length()-1) 
								|| (dan.indexOf(x) + x.length() + 1 != dan.length()-1 
								&& dan.charAt(dan.indexOf(x) + x.length() + 2) == '*'))//若x为二次幂
						{
							for (i = 0; i < dan.length() 
									&& dan.charAt(i) >= '0' 
									&& dan.charAt(i) <= '9'; ){
								i++;
							}
							...
							resu = String.valueOf(mi*2) 
									+ dan.substring(i, dan.indexOf(x)) + x 
									+ dan.substring(dan.indexOf(x) + x.length() + 2);
					
						}
						else
						{
							for (i = dan.indexOf(x) + x.length() + 1; 
									i < dan.length() && dan.charAt(i) != '*';){
								i++;
							}
							mi = Integer.parseInt(dan.substring(
									dan.indexOf(x) + x.length() + 1, i));//幂次
							for (i = 0; i < dan.length() 
									&& dan.charAt(i) >= '0' && dan.charAt(i) <= '9';){
								i++;
							}
							xishu = Integer.parseInt(
									dan.substring(0, i));//系数
							resu = String.valueOf(mi*xishu) 
									+ dan.substring(i, dan.indexOf(x)) + x + '^' 
									+ String.valueOf(mi-1) 
									+ dan.substring(dan.indexOf(x) + x.length() 
									+ 1 + String.valueOf(mi-1).length());
					...
					else//开头没系数
					{
						...
						{
							for (i = dan.indexOf(x) + x.length() + 1; 
									i < dan.length() && dan.charAt(i) != '*'; i++);
							mi = Integer.parseInt(
									dan.substring(dan.indexOf(x) + x.length() + 1, i));//幂次
							if (fine == 1)
								news2 = news2 + '-';
							resu = String.valueOf(mi) + '*' 
									+ dan.substring(0, dan.indexOf(x)) + x + '^' 
									+ String.valueOf(mi-1) + dan.substring(dan.indexOf(x) 
									+ x.length() + 1 + String.valueOf(mi-1).length());
						}
					}
				}
		}
